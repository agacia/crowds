...
public class MySandSharc5 extends DecentralizedCommunityAlgorithm {
	...
	public MySandSharc5(Graph graph) {
		super(graph);
	}
...
//			System.out.println("comStrength " + comStrength);
			Double comStrengthThreshold = 0.1;
			if (maxScore < comStrengthThreshold) {
				originateCommunity(u);
//				System.err.println("originate\t" + u.getId() + "," + currentCommunity + "\tnewcom\t" + u.getAttribute(marker) + currentCommunity + "\tmaxScore\t" + maxScore);
			}
			else {
				u.setAttribute(marker, maxCommunity);
//				System.err.println("join neighbors\t" + u.getId() + "," + currentCommunity + "\tnewcom\t" + maxCommunity + "," + maxScore);
			}
